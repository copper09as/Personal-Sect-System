...
public static class NetManager
{
    ...
    private static void ReceiveCallback(IAsyncResult ar)
    {
		...
		{
			MessageBox.Show("Socket Receive Fail" + ex.ToString());
    ...
    private static void OnReceiveDate()
    {
        ...
        if (protoName == "")
		{
			MessageBox.Show("OnReceiveData Msg.DecodeName Fail");
			return;
		}
    ...
    public static void Close()
    {
        ...
        {
            Debug.WriteLine($"关闭连接异常: {ex.Message}");
        }
        finally
        {
            socket = null;
    ...
    private static void SendCallBack(IAsyncResult ar)
    {
        ...
        {
            Debug.WriteLine($"发送回调异常（连接已关闭）: {ex.Message}");
        }
        catch (SocketException ex)
        {
            Debug.WriteLine($"发送失败: {ex.SocketErrorCode}");
            Close();
        }
        catch (InvalidOperationException ex)
        {
            Debug.WriteLine($"队列操作异常: {ex.Message}");
        }
    }
